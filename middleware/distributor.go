...
func Distribute() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		userSetting, settingFound := common.GetContextKeyType[dto.UserSetting](c, constant.ContextKeyUserSetting)
		if !settingFound {
			userSetting, _ = model.GetUserSetting(c.GetInt("id"), false)
		}
		requestGroup := getEffectiveRequestGroup(c, modelRequest)
		if requestGroup != "" && !service.GroupInUserUsableGroupsWithSetting(userGroup, requestGroup, userSetting) {
		...
		if ok {
			...
			if shouldSelectChannel {
				...
				if usingGroup != "" && !service.GroupInUserUsableGroupsWithSetting(userGroup, usingGroup, userSetting) {
					abortWithOpenAiMessage(c, http.StatusForbidden, i18n.T(c, i18n.MsgDistributorGroupAccessDenied))
					return
				}
...
func getEffectiveRequestGroup(c *gin.Context, modelRequest *ModelRequest) string {
	requestGroup := common.GetContextKeyString(c, constant.ContextKeyUsingGroup)
	if strings.HasPrefix(c.Request.URL.Path, "/pg/chat/completions") && modelRequest.Group != "" {
		// Session auth initializes the using group from the user's account group.
		// Playground requests explicitly select their model group in the body.
		return modelRequest.Group
	}
	return requestGroup
}
